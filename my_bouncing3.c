...
void my_plot_objects(Object objs[], const size_t numobj, const double t, const Condition cond, int *check);
...
int main(int argc, char **argv){
  ...
  int check;
  printf("\n");
  my_plot_objects(objects, objnum, t, cond,&check);
  for (int i = 0 ; t <= stop_time ; i++){
    ...
    printf("\e[%dA", cond.height+3+check);// 壁とパラメータ表示分で3行
  }
  return EXIT_SUCCESS;
}
  ...
  for(int j = 0; j < cond.height; j++){
    ...
    for(int i = 0; i < cond.width; i++){
      ...
      for( int k = 0; k < numobj;k++){
        if( (i == (int)(zahyou[k][0] + x0)) && (j == (int)(zahyou[k][1] + y0)) && (objs[k].m != 0) ){
...
void my_combine(Object objs[], const size_t numobj, const Condition cond){
    for(int i = 0; i < numobj;i++){
        for(int j = 0; j < numobj; j++){
            if(i != j){
                double distance_zettaiti =sqrt(pow((objs[i].y - objs[j].y),2) +pow((objs[i].x - objs[j].x),2));
                if((distance_zettaiti <= 2) && (objs[i].m != 0) && (objs[j].m != 0)){
