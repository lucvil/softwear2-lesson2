...
void my_write_objects(Object objs[],const size_t numobj,FILE *fp);
...
//ファイルの読み込み
  if ( argc > 3 || argc < 3) {
    fprintf(stderr, "usage: %s  [number] [filename for init]\n", argv[0]);
    return EXIT_FAILURE;
  }else if (argc == 3) {
      //初期化
    FILE *lgfile;
    if ( (lgfile = fopen(argv[2],"r")) != NULL ) {
      my_write_objects(objects,objnum,lgfile); // ファイルによる初期化
    }    
    else{
...
void my_write_objects(Object objs[],const size_t numobj,FILE *fp){
    ...
    while((fgets(copy,100,fp)) != NULL){
        if(copy[0] != '#'){
            ...
            sscanf(copy1,"%lf %lf %lf %lf %lf", &objs[i].m, &objs[i].x, &objs[i].y, &objs[i].vx, &objs[i].vy);
